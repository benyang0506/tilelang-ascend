...
The current CI workflow is triggered manually via [`bench_test.sh`](./examples/bench_test.sh), which:
- Auto-discovers and runs all `*.py` scripts under `examples/` (up to depth 2).
- Executes Flash Attention optimization benchmarks under `examples/flash_attention/fa_opt/`.
- Runs AOT compilation tests and PyTorch integration tests.
- Runs `pytest` on all tests under `testing/python/`.
- Reports a pass/fail summary with pass rate.
