...
add_test_case() {
    local script_path="$1"
    local script_args="${2:-}"
    all_scripts+=("${script_path}|${script_args}")
}
...
if [ -n "$python_files" ]; then
    for file in $python_files; do add_test_case "$file"; done
fi
...
if [ -d "$fa_dir" ]; then
    ...
    if [ -n "$fa_python_files" ]; then
        for file in $fa_python_files; do add_test_case "$file"; done
    fi
fi

# ===== add examples/sparse_flash_attention/bench_sfa/bench_sfa.py with 4 variants =====
sfa_bench_script="./sparse_flash_attention/bench_sfa/bench_sfa.py"
if [ -f "$sfa_bench_script" ]; then
    sfa_variants=(
        "sparse_flash_attn_pa_baseline"
        "sparse_flash_attn_pa_developer"
        "sparse_flash_attn_pa_no_cv_pipeline"
    )
    for variant in "${sfa_variants[@]}"; do
        add_test_case "$sfa_bench_script" "--file=$variant"
    done
fi
...
if [ -d "./gemm_aot" ]; then
    ...
    if [ -n "$bash_scripts" ]; then
        for script in $bash_scripts; do add_test_case "$script"; done
    fi
fi
...
for test_case in "${all_scripts[@]}"; do
    total_scripts=$((total_scripts + 1))

    # 启动后台子进程
    {
        IFS='|' read -r script script_args <<< "$test_case"
        script_dir=$(dirname "$script")
        script_name=$(basename "$script")
        display_name="$script"
        if [ -n "$script_args" ]; then
            display_name="$script $script_args"
        fi

        # 执行脚本并捕获输出到变量，不在磁盘生成日志文件
        if [[ "$script" == *.py ]]; then
            if [ -n "$script_args" ]; then
                output=$(cd "$script_dir" && python "$script_name" "$script_args" 2>&1)
            else
            ...
            if [ -n "$script_args" ]; then
                output=$(cd "$script_dir" && bash "$script_name" "$script_args" 2>&1)
            else
        ...
        if [[ "$output" =~ [Kk][Ee][Rr][Nn][Ee][Ll][[:space:]][Oo][Uu][Tt][Pp][Uu][Tt][[:space:]][Mm][Aa][Tt][Cc][Hh] ]] || [[ "$output" =~ [Tt][Ee][Ss][Tt][[:space:]][Pp][Aa][Ss][Ss][Ee][Dd][!] ]]; then
            echo "[PASSED] $display_name"
            touch "$temp_dir/pass_$total_scripts"
        else
            echo "[FAILED] $display_name (Exit: $exit_code)"
